...
@Singleton
class BukkitLegacyTextFormatter @Inject constructor(
) {

    fun format(text: BukkitFcText): String {
        val state = FormatterState()
        appendFormat(text, state)
        return state.stringBuilder.toString()
    }

    private fun appendFormat(text: BukkitFcText, details: FormatterState) {
