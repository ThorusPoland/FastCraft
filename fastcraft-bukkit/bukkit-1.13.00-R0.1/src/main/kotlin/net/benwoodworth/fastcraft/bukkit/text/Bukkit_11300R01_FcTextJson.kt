...
@Suppress("unused")
@Serializable
class Bukkit_11300R01_FcTextJson(
    ...
    @SerialName("color")
    val color: String?
        get() = fcText.color?.`as`<Bukkit_11300R01_FcTextColor>()?.name
    ...
    @SerialName("extra")
    val extra: List<Bukkit_11300R01_FcTextJson>?
        get() = fcText.extra
            .takeIf { it.any() }
            ?.map { Bukkit_11300R01_FcTextJson(it) }
}